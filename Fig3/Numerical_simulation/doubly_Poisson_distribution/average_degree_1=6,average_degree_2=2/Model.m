...
while(step<=time)
    ...
    add_amount=sum(sum(S_Y_set{step+1}.*(1-(1-beta*eta).^index_set_3)));
    num_Q_S_1=sum(sum(index_set_1.*S_Y_set{step+1}.*(1-(1-beta*eta).^index_set_3)));
    den_Q_S_1=sum(sum(index_set_1.*S_Y_set{step+1}));
    num_Q_S_2=sum(sum(index_set_2.*S_Y_set{step+1}.*(1-(1-beta*eta).^index_set_3)));
    den_Q_S_2=sum(sum(index_set_2.*S_Y_set{step+1}));
    num_Q_I_1=sum(sum(index_set_3.*S_Y_set{step+1}.*(1-(1-beta*eta).^index_set_3)));
    den_Q_I_1=sum(sum(index_set_1.*I_set{step+1}));
    num_Q_I_2=sum(sum(index_set_4.*S_Y_set{step+1}.*(1-(1-beta*eta).^index_set_3)));
...
%             I_new(t1,t2)=sum(sum(I_set{step+1}(index_alt_1,index_alt_2).*H2));
    ...
    num_P_S_1=sum(sum(index_set_1.*S_Y_set{step}.*(1-(1-alpha*lambda0).^(index_set_3+index_set_4))));
    den_P_S_1=sum(sum(index_set_1.*S_Y_set{step}));
    num_P_S_2=sum(sum(index_set_2.*S_Y_set{step}.*(1-(1-alpha*lambda0).^(index_set_3+index_set_4))));
    den_P_S_2=sum(sum(index_set_2.*S_Y_set{step}));
    num_P_I_1=sum(sum(index_set_3.*S_Y_set{step}.*(1-(1-alpha*lambda0).^(index_set_3+index_set_4))));
    den_P_I_1=sum(sum(index_set_1.*I_set{step}));
    num_P_I_2=sum(sum(index_set_4.*S_Y_set{step}.*(1-(1-alpha*lambda0).^(index_set_3+index_set_4))));
...
%             n1=index_set_3(index_alt_1,index_alt_2);n2=index_set_4(index_alt_1,index_alt_2);
            ...
            S_Y_set{step+1}(t1,t2)=sum(S_Y_set{step}(index)...
                .*((1-alpha*lambda0).^(n1+n2)).*G1);
            I_set{step+1}(t1,t2)=sum(S_Y_set{step}(index)...
                .*(1-(1-alpha*lambda0).^(n1+n2)).*G1+I_set{step}(index).*(1-gamma).*G2);
